...
from data_types.data_types import TimedPathPair, Pair
...
class FilesManager:
    ...
    def update_from_config(self, config: Config):
        ...
        self.set_delta_time_per_image(delta_time_per_image)
        if self.is_valid():
            self.create_list_timed_matching_image_path_pair()

    def set_delta_time_per_image(self, new_delta_time_per_image):
    ...
    def get_images_paths_from_camera(self, camera_identifier: CameraIdentifier) -> List[str]:
        images_paths = []
    ...
    def create_list_timed_matching_image_path_pair(self, starting_point=None) -> None:
        if starting_point:
        ...
        for idx in range(maximum_number_of_pairs):
            ...
            self.list_timed_matching_image_path_pair.append(timed_matching_path_pair)

            timestamp += self._delta_time_per_image

    def _get_pair_lists_of_sorted_images_path(self)-> Tuple:

        list_images_paths_left_camera = self.get_images_paths_from_camera(CameraIdentifier.LEFT_CAMERA)
        list_images_paths_right_camera = self.get_images_paths_from_camera(CameraIdentifier.RIGHT_CAMERA)
    ...
    def is_valid(self):
        if self._directory_images_left_camera is None or self._directory_images_right_camera is None or self._delta_time_per_image is None:
            return False
